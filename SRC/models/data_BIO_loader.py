...
def convert_examples_to_features(args, train_instances, max_span_length=8):
    ...
    for ex_index, example in tqdm(enumerate(train_instances), total=len(train_instances), desc="Processing", unit="example"):
...
def load_pos_data(args, src_pos_data, if_train=False):
    pos_feature = []
    if if_train:
        random.seed(args.seed)
        random.shuffle(src_pos_data)
    for ex_index, example in tqdm(enumerate(src_pos_data), total=len(src_pos_data), desc="Processing",
                                  unit="example"):
        sample = {'id': ex_index}
        temp = src_pos_data[ex_index]
        sample['pos_input_ids'] = temp['pos_input_ids']
        sample['pos_attention_mask'] = temp['pos_attention_mask']
        sample['pos_token_type_ids'] = temp['pos_token_type_ids']
        sample['pos_span_label'] = temp['pos_span_label']
        sample['pos_span_mask'] = temp['pos_span_mask']
        pos_feature.append(sample)
    list_instance_batch = []
    for i in range(0, len(pos_feature), args.train_batch_size):
        list_instance_batch.append(pos_feature[i:i + args.train_batch_size])
    return list_instance_batch

def load_dp_data(args, src_dp_data, if_train=False):
    dp_feature = []
    if if_train:
        random.seed(args.seed)
        random.shuffle(src_dp_data)
    for ex_index, example in tqdm(enumerate(src_dp_data), total=len(src_dp_data), desc="Processing",
                                  unit="example"):
        sample = {'id': ex_index}
        temp = src_dp_data[ex_index]
        sample['dp_input_ids'] = temp['input_ids']
        sample['dp_attention_mask'] = temp['attention_mask']
        sample['dp_token_type_ids'] = temp['token_type_ids']
        sample['dp_span_label'] = temp['span_label']
        sample['dp_span_mask'] = temp['span_mask']
        dp_feature.append(sample)
    list_instance_batch = []
    for i in range(0, len(dp_feature), args.train_batch_size):
        list_instance_batch.append(dp_feature[i:i + args.train_batch_size])
    return list_instance_batch
    ...
    for ex_index, line in enumerate(lines):
        ...
        for triple in raw_pairs:
            ...
            if len(raw_aspect) == 1:
                if raw_aspect[0] == -1:
                    raw_aspect[0] = 0
                    aspect_word = sentence[raw_aspect[0]]  # 这里是记录方面术语是什么单词
                    raw_aspect = [raw_aspect[0], raw_aspect[0]]  # 这里是记录方面术语的起始位置和结束位置，例如[3,3]
                else:
            ...
            aspect_label = {}  # 这里面是先定义一个字典，然后将单词定义为关键字，后面是单词起始位置和结束位置的索引，{'blushes': [3, 3]}
            aspect_label[aspect_word] = [raw_aspect[0], raw_aspect[-1]]
            aspect_num += len(aspect_label)  # 记录方面标签的长度

            if len(raw_opinion) == 1:
                opinion_word = sentence[raw_opinion[0]]  # 记录当前意见术语的单词
                raw_opinion = [raw_opinion[0], raw_opinion[0]]  # 记录当前意见术语的起始位置和结束位置的索引
            else:
                opinion_word = ' '.join(sentence[raw_opinion[0]: raw_opinion[-1] + 1])
            opinion_label = {}  # 定义标签字典
            opinion_label[opinion_word] = [raw_opinion[0], raw_opinion[-1]]

            word = str(aspect_word) + '|' + str(opinion_word)  # 拼接一下方面术语和意见术语
            if word not in triple_dict:  # 将拼接的结果加入到三元组字典中
                triple_dict[word] = []
                triple_dict[word] = ([raw_aspect[0], raw_aspect[-1]], [raw_opinion[0], raw_opinion[-1]], sentiment)  # 记录方面术语、意见术语和情感极性
            else:
                print('单句' + id + '中三元组重复出现！')
        examples = InputExample(id=id, text_a=line[0], text_b=None, all_label=triple_dict, aspect_num=aspect_num,
                                triple_num=len(triple_dict))  # 这里结束之后是一句话循环结束
...
class DataTterator(object):
    ...
    def get_dp_batch(self, batch_num):
        dp_input_ids, dp_attention_mask, dp_token_type_ids, dp_span_label, dp_span_mask = [], [], [], [], []
        final_dp_input_ids, final_attention_mask, final_token_type_ids, final_span_label, final_span_mask = None, None, None, None, None,
        if batch_num <= len(self.instances):
            for i, sample in enumerate(self.instances[batch_num]):
                input_ids = sample['dp_input_ids']
                attention_mask = sample['dp_attention_mask']
                token_type_ids = sample['dp_token_type_ids']
                span_label = sample['dp_span_label']
                span_mask = sample['dp_span_mask']
                input_ids_tensor = torch.tensor([input_ids])
                attention_mask_tensor = torch.tensor([attention_mask])
                token_type_ids_tensor = torch.tensor([token_type_ids])
                span_label_tensor = torch.tensor([span_label])
                span_mask_tensor = torch.tensor([span_mask])
                dp_input_ids.append(input_ids_tensor)
                dp_attention_mask.append(attention_mask_tensor)
                dp_token_type_ids.append(token_type_ids_tensor)
                dp_span_label.append(span_label_tensor)
                dp_span_mask.append(span_mask_tensor)
            for dp_input, dp_attention, dp_token_type, dp_span_l, dp_span_m in zip(dp_input_ids, dp_attention_mask, dp_token_type_ids, dp_span_label, dp_span_mask):
                if final_dp_input_ids == None:
                    final_dp_input_ids = dp_input
                    final_attention_mask = dp_attention
                    final_token_type_ids = dp_token_type
                    final_span_label = dp_span_l
                    final_span_mask = dp_span_m
                else:
                    final_dp_input_ids = torch.cat((final_dp_input_ids, dp_input), dim=0)
                    final_attention_mask = torch.cat((final_attention_mask, dp_attention), dim=0)
                    final_token_type_ids = torch.cat((final_token_type_ids, dp_token_type), dim=0)
                    final_span_label = torch.cat((final_span_label, dp_span_l), dim=0)
                    final_span_mask = torch.cat((final_span_mask, dp_span_m), dim=0)
            final_dp_input_ids = final_dp_input_ids.to(self.args.device)
            final_attention_mask = final_attention_mask.to(self.args.device)
            final_token_type_ids = final_token_type_ids.to(self.args.device)
            final_span_label = final_span_label.to(self.args.device)
            final_span_mask = final_span_mask.to(self.args.device)

            return final_dp_input_ids, final_attention_mask, final_token_type_ids, final_span_label, final_span_mask

    def get_pos_batch(self, batch_num):
        pos_input_ids, pos_attention_mask, pos_token_type_ids, pos_span_label, pos_span_mask = [], [], [], [], []
        final_pos_input_ids, final_pos_attention_mask, final_pos_token_type_ids, final_pos_span_label, final_pos_span_mask = None, None, None, None, None,
        if batch_num <= len(self.instances):
            for i, sample in enumerate(self.instances[batch_num]):
                input_ids = sample['pos_input_ids']
                attention_mask = sample['pos_attention_mask']
                token_type_ids = sample['pos_token_type_ids']
                span_label = sample['pos_span_label']
                span_mask = sample['pos_span_mask']
                input_ids_tensor = torch.tensor([input_ids])
                attention_mask_tensor = torch.tensor([attention_mask])
                token_type_ids_tensor = torch.tensor([token_type_ids])
                span_label_tensor = torch.tensor([span_label])
                span_mask_tensor = torch.tensor([span_mask])
                pos_input_ids.append(input_ids_tensor)
                pos_attention_mask.append(attention_mask_tensor)
                pos_token_type_ids.append(token_type_ids_tensor)
                pos_span_label.append(span_label_tensor)
                pos_span_mask.append(span_mask_tensor)
            for pos_input, pos_attention, pos_token_type, pos_span_l, pos_span_m in zip(pos_input_ids, pos_attention_mask, pos_token_type_ids, pos_span_label, pos_span_mask):
                if final_pos_input_ids == None:
                    final_pos_input_ids = pos_input
                    final_pos_attention_mask = pos_attention
                    final_pos_token_type_ids = pos_token_type
                    final_pos_span_label = pos_span_l
                    final_pos_span_mask = pos_span_m
                else:
                    final_pos_input_ids = torch.cat((final_pos_input_ids, pos_input), dim=0)
                    final_pos_attention_mask = torch.cat((final_pos_attention_mask, pos_attention), dim=0)
                    final_pos_token_type_ids = torch.cat((final_pos_token_type_ids, pos_token_type), dim=0)
                    final_pos_span_label = torch.cat((final_pos_span_label, pos_span_l), dim=0)
                    final_pos_span_mask = torch.cat((final_pos_span_mask, pos_span_m), dim=0)
            final_pos_input_ids = final_pos_input_ids.to(self.args.device)
            final_pos_attention_mask = final_pos_attention_mask.to(self.args.device)
            final_pos_token_type_ids = final_pos_token_type_ids.to(self.args.device)
            final_pos_span_label = final_pos_span_label.to(self.args.device)
            final_pos_span_mask = final_pos_span_mask.to(self.args.device)

            return final_pos_input_ids, final_pos_attention_mask, final_pos_token_type_ids, final_pos_span_label, final_pos_span_mask
